...
class MtgFciL1cConfig(BasePluginConfig):
    ...
    def __post_init__(self) -> None:
        """Validate operator-facing options at config-construction time.

        Each concern is delegated to a small ``_validate_*`` method. Order
        matters: ``_validate_zarr_chunk_y`` reads ``zarr_chunk_overrides`` to
        skip resolutions that also have an explicit chunk override, so the
        override-shape checks run afterwards. Override divisibility against
    ...
    def _validate_pixel_time_dtype(self) -> None:
    ...
    def _validate_projection_units(self) -> None:
    ...
    def _validate_zarr_chunk_y(self) -> None:
        """Reject ``zarr_chunk_y`` values above the per-resolution assembly cap.

        Resolutions that already carry an explicit ``zarr_chunk_overrides``
        entry are skipped here: their chunk shape is validated in
        ``_validate_zarr_chunk_overrides`` instead.
        """
        if self.zarr_chunk_y is None:
            return
    ...
    def _validate_zarr_shard_overrides(self) -> None:
        if self.zarr_shard_overrides is None:
            return
    ...
    def _validate_zarr_chunk_overrides(self) -> None:
        if self.zarr_chunk_overrides is None:
            return
